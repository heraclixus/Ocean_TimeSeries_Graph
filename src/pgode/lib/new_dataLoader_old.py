...
class ParseData(object):
    ...
    def __init__(self, dataset_path,args,suffix='_springs5',mode="interp"):
        ...
        self.num_condition = args.condition_num  # 12
        print("num_condition:{}, num_predict:{}, total_step:{}".format(self.num_condition, self.num_pre, self.total_step))
    ...
    def load_data(self,sample_percent,batch_size,data_type="train"):
        ...
        sys_paras = np.load(self.dataset_path + '/paras_' + data_type + self.suffix + '.npy', allow_pickle=True)[
                :cut_num]  # [500,4]
        ...
        if mode =="interp":
            ...
            for i in range(self.num_graph):
                for j in range(self.num_atoms):
                    ...
                    loc_extrap[i][j] = loc[i][j][self.num_condition:self.total_step]
                    vel_extrap[i][j] = vel[i][j][self.num_condition:self.total_step]
                    times_extrap[i][j] = times[i][j][self.num_condition:self.total_step]
    ...
    def transfer_one_graph(self,loc, vel, edge, time, time_begin=0, mask=True, forward=False):
        ...
        if self.cutting_edge:
            if self.suffix == "_springs5" or self.suffix == "_charged5" or self.suffix == "_springs10" or self.suffix == "_charged10":
                ...
                max_gap = (self.total_step - self.num_pre * self.sample_percent) / self.total_step
            else:
